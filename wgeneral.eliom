...
let make_draggable ?(label=Js.string "text") dom_ref (elem,info) = 
  ...
  List.iter (fun (ev,f) -> Lwt.async (fun () -> (ev : ?use_capture:'a -> 'b)  elem f))
...
let make_dropzone dom_ref 
  ...
  let ondrop ev _  =
	...
	opt_iter (fun (elem, draginfo) ->
	  ...
	  Dom.appendChild container elem) elem ;
	Lwt.return () in
  ...
  (* The type annotation is necessary because of a limitation in optional arguments *)
  List.iter (fun (ev,f) -> Lwt.async (fun () -> (ev : ?use_capture:'a -> 'b) dropzone f))
