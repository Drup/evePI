...
{client{
let rec add_new_planets uname picon = function
  | [] -> [uname,[picon]]
  | (h,p) :: t when h = uname -> (h, (picon :: p)) :: t
  | h :: t -> h :: add_new_planets uname picon t

let rec generate_node_content = function
  | [] -> []
  | (user_name, planets) :: l ->
      let planets =
	match planets with
	  | [] -> [span []]
	  | _ -> planets
      in D.dt [pcdata user_name] :: D.dd planets :: generate_node_content l
 }}
...
let admin_project_tree project user =
  ...
  let extract_planets planets =
    List.flatten (
      List.map
	(fun ((uid,uname),l) ->
	   List.map (fun (pid,picon) -> (picon,(pid,uid,uname))) l)
	planets )
  in
  ...
  let _ = {unit{
      let draggables =
	List.map
	  (fun (p,(pid,uid,uname)) -> (To_dom.of_element p, (pid,uid,uname)))
	%draggables
      in
      let dropzones =
	List.map
	  (fun (id, container, user_content) ->
	     let f planet (pid,uid,uname) =
	       let user_planets = To_dom.of_dd (List.assoc uid user_content) in
